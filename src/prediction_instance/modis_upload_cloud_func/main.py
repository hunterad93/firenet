...
def get_modis_data(api_key, bbox, date=None):
    '''
    Connect with FIRMS API to access modis detection data from a specified date and the day before
    ...
    :return: GeoDataFrame of modis detection data with columns corresponding to the FIRMS API response
    '''
    ...
    for date in dates:
        url = f'{base_url}{api_key}/MODIS_NRT/{bbox}/1/{date}'
    ...
    modis_data = pd.concat(data_frames, ignore_index=True)

    # Convert the DataFrame to a GeoDataFrame, setting the geometry from the latitude and longitude columns
    gdf = gpd.GeoDataFrame(modis_data, geometry=gpd.points_from_xy(modis_data.longitude, modis_data.latitude))
...
def filter_clusters(gdf, min_cluster_size=5, min_high_confidence=1):
    """
    Filter out clusters that have fewer points, and fewer high confidence points, than the two thresholds
    and exclude the 'noise' cluster labeled as -1 by DBSCAN.
    ...
    :return: GeoDataFrame of fire points in clusters that meet both thresholds and are not considered 'noise'
    """
    # Exclude the 'noise' cluster with label -1
    print(len(gdf.index))
    gdf = gdf[gdf['label'] != -1]
    print(len(gdf.index))
    ...
    high_confidence_counts = gdf.loc[gdf['confidence'] > 50]['label'].value_counts()

    # Filter out small clusters and clusters with too few high confidence points
    # Ensure to handle cases where a cluster might not have any high confidence points
    valid_clusters = cluster_counts[(cluster_counts >= min_cluster_size) & (high_confidence_counts.reindex(cluster_counts.index, fill_value=0) >= min_high_confidence)].index
...
def create_cluster_polygons(gdf):
    ...
    if polygons.empty:
        # Handle the case where there are no polygons (e.g., no clusters or all clusters filtered out)
        print('no polygons')
    else:
        # Create a new GeoDataFrame from the polygons with an explicit index if polygons are not empty
        polygon_gdf = gpd.GeoDataFrame({'geometry': polygons}, index=polygons.index)
...
def upload_to_bigquery(acq_date, polygon_geojson):
    ...
    row = {
        ...
        'modis_mask_geojson': polygon_geojson,
...
def MODIS_GEOJSON_UPDATE(request):
    ...
    # Get the modis data
    modis_data = get_modis_data(api_key, bbox)

    # Filter out points from the last 24 hours
    modis_data = filter_last_24_hours(modis_data)

    # Cluster the fire points
    clustered_fires = cluster_fires(modis_data)
    # Delete modis_data as it's no longer needed
    del modis_data
