...
def cluster_fires(fire_dataframe, min_cluster_points=25, timescale = 'day'):
    """
    ...
    :param timescale: str, either 'day' or 'hour', indicates from which timescale to cluster points
    :return: geodataframe of fire points that belong to a cluster
    """
    clustered_fires_for_dates = []
    number_of_clusters = 0

    # Link to specific column depending on user preference
    if timescale == 'day':
        unique_id = 'acq_date'
    elif timescale == 'hour':
        unique_id = 'acq_date_hour'

    for date in fire_dataframe[unique_id].unique().tolist():
        fires_for_date = fire_dataframe[fire_dataframe[unique_id] == date]


        if fires_for_date.empty == True:
            continue
    ...
    # If list is empty
    if not clustered_fires_for_dates:
        return -1
...
def create_chip_bounds(clustered_fires):
    ...
    for cluster in clustered_fires["label"].unique().tolist():
        ...
        date = clustered_fire["acq_date_hour"].values[0]
...
def fires_from_topleft(chip_bounds, fires, fname_suffix = '', 
    ...
    for each_date in list_of_dates:
        ...
        for _, sample in samples_in_that_day.iterrows():
            ...
            coords = [sample['left'],sample['right'], sample['bottom'], sample['top']]
            epsg_code = str(sample['epsg'])
            date_to_query = sample['date']
                                        
                                       
            # Appears that AOI is going to be the same size every time.
            ...
            if isinstance(fires, str):
                ...
                fires_in_chip = fires[fires["acq_date_hour"] == date_to_query].clip(chip_poly)

            fires_in_chip = fires_in_chip[fires_in_chip["acq_date_hour"] == date_to_query]
...
##            if output_raster_img is None:
            ...
            if export == True:
                ...
                output.attrs = {}
                output.to_netcdf(path + "VIIRS_Rasterized_" + str(each_date) + fname_suffix + str(num_fires) + '_' + variable + '.nc')
...
##        if export == True:
##            num_fires = 1
##            fname_suffix  = '_Num' + str(num_fires) + fname_suffix
##
##            if variable == "bool":
...
##                    num_fires += 1
##                    fname_suffix = fname_suffix[:4] + str(num_fires) + fname_suffix[5:]
##                
##                if variable == "bool":
##                    output = output_raster_img.bool
##                    output.attrs = {}
##                    output.to_netcdf(path + "VIIRS_Rasterized_" + str(each_date) + fname_suffix + '.nc')
##
##                else: 
##                    output = output_raster_img.frp
##                    output.attrs = {}
